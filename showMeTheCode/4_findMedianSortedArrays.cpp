//
// Created by chuanxu on 2021/6/30.
//
#include <iostream>
#include <vector>
...
//方法2
    ...
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        if (totalLength % 2 == 1) {
            return findKthElem(nums1, nums2, (totalLength + 1) / 2);
        }
        else {
            return (findKthElem(nums1, nums2, totalLength / 2) + findKthElem(nums1, nums2, totalLength / 2 + 1)) / 2.0;
        }
    }
};
...
int main()
{
    ...
    cout << "mid number = " << solution.findMedianSortedArrays(nums1, nums2) << endl;
    return 0;
}